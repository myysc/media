...
import socket  # 添加socket模块用于域名解析
...
# 第三阶段
def third_stage():
    print("🧩 第三阶段：多线程检测代表频道生成 zbtv.txt 并写回可用 IP 到 ip/目录（覆盖）")
    
    # 新增：清空ip目录下的所有txt文件，避免旧数据积累
    print("🧹 清空ip目录下的所有txt文件...")
        ...
        for filename in os.listdir(IP_DIR):
            if filename.endswith(".txt"):
                ...
                try:
                    ...
                    print(f"⚠️ 删除文件 {filename} 失败：{e}")
    else:
        os.makedirs(IP_DIR, exist_ok=True)
    
    print("✅ ip目录已清空，准备重新写入有效IP")
    ...
    # 读取zubo.txt并建立分组（不再读取现有ip文件，因为已清空）
    groups = {}
    ...
    for ip_port in playable_ips:
        ...
        try:
            ...
            operator = f"{province}{isp}"
        except Exception as e:
            print(f"⚠️ 获取 {ip_port} 运营商信息失败：{e}")
            operator = "未知"
        ...
        try:
            with open(target_file, "w", encoding="utf-8") as wf:
                for ip_p in sorted(ip_set):
                    wf.write(ip_p + "\n")
            print(f"📥 写入 {target_file}，共 {len(ip_set)} 个可用地址")
        except Exception as e:
            print(f"❌ 写入 {target_file} 失败：{e}")

    # 写 zbtv.txt（包含更新时间与分类）
                ...
                for ch in ch_list:
                    for line in valid_lines:
                        ...
                        if name == ch:
                            f.write(line + "\n")
                f.write("\n")
        print(f"🎯 zbtv.txt 生成完成，共 {len(valid_lines)} 条频道")
    except Exception as e:
        print(f"❌ 写 zbtv.txt 失败：{e}")
...
def push_all_files():
    ...
    os.system('git commit -m "自动更新：计数、IP文件" || echo "⚠️ 无需提交"')
